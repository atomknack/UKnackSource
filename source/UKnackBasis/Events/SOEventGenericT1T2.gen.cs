...
public abstract class SOEvent<T1,T2> : ScriptableObjectWithReadOnlyName, IEvent<T1,T2>
{
    [NonSerialized] internal readonly List<UnityEvent<T1,T2>> subscribedByUnityEvent = new List<UnityEvent<T1,T2>>();
    [NonSerialized] internal readonly List<Action<T1,T2>> subscribedByCSEvent = new List<Action<T1,T2>>();
    [NonSerialized] internal readonly List<ISubscriberToEvent<T1,T2>> subscribedByISubscriberToEvent = new List<ISubscriberToEvent<T1,T2>>();
#if UNITY_EDITOR
    ...
    public virtual void Subscribe(Action<T1,T2> subscriber) 
    {
        if (subscriber == null)
...
var method= new StackFrame(1).GetMethod();
    ...
    internal virtual void Unsubscribe(Action<T1,T2> subscriber)
    {
#if UNITY_EDITOR
if (subscriber == null)
    throw new ArgumentNullException(nameof(subscriber));
...
if (removed == false)
    ...
    void IEvent<T1,T2>.Unsubscribe(Action<T1,T2> subscriber) => 
        Unsubscribe(subscriber);

    internal virtual bool IsOneOfSubscribers(Action<T1,T2> subscriber)
    {
        if (subscriber == null)
            throw new ArgumentNullException(nameof(subscriber));
    ...
    /// <param name="subscriber">To unsubscribe use UnsubscribeNullSafe extension method.
    /// subscriber will be called when something is published to event</param>
    public virtual void Subscribe(UnityEvent<T1,T2> subscriber) 
    {
        if (subscriber == null)
            throw new ArgumentNullException(nameof(subscriber));
...
var method= new StackFrame(1).GetMethod();
    ...
    internal virtual void Unsubscribe(UnityEvent<T1,T2> subscriber)
    {
#if UNITY_EDITOR
if (subscriber == null)
    throw new ArgumentNullException(nameof(subscriber));
...
if (removed == false)
    ...
    void IEvent<T1,T2>.Unsubscribe(UnityEvent<T1,T2> subscriber) => 
        Unsubscribe(subscriber);

    internal virtual bool IsOneOfSubscribers(UnityEvent<T1,T2> subscriber)
    {
        if (subscriber == null)
            throw new ArgumentNullException(nameof(subscriber));
    ...
    /// <param name="subscriber">To unsubscribe use UnsubscribeNullSafe extension method.
    /// subscriber will be called when something is published to event</param>
    public virtual void Subscribe(ISubscriberToEvent<T1,T2> subscriber) 
    {
        if (subscriber == null)
            throw new ArgumentNullException(nameof(subscriber));
...
var method= new StackFrame(1).GetMethod();
    ...
    internal virtual void Unsubscribe(ISubscriberToEvent<T1,T2> subscriber)
    {
#if UNITY_EDITOR
if (subscriber == null)
    throw new ArgumentNullException(nameof(subscriber));
...
if (removed == false)
    ...
    void IEvent<T1,T2>.Unsubscribe(ISubscriberToEvent<T1,T2> subscriber) => 
        Unsubscribe(subscriber);

    internal virtual bool IsOneOfSubscribers(ISubscriberToEvent<T1,T2> subscriber)
    {
        if (subscriber == null)
            throw new ArgumentNullException(nameof(subscriber));
    ...
    internal virtual void InternalInvoke(T1 t1,T2 t2)
    {
        InvokeSubscribers(this, t1,t2);
    }

    internal static void InvokeSubscribers(SOEvent<T1,T2> soe, T1 t1,T2 t2)
    {
        if (soe==null)
            throw new ArgumentNullException(nameof(soe));
        //internalEvent.Publish(t1,t2);

        for (int i = 0; i<soe.subscribedByCSEvent.Count; ++i)
        {
            soe.subscribedByCSEvent[i]?.Invoke(t1,t2);
        }
        for (int i = 0; i<soe.subscribedByUnityEvent.Count; ++i)
        {
            soe.subscribedByUnityEvent[i]?.Invoke(t1,t2);
        }
        for (int i = 0; i<soe.subscribedByISubscriberToEvent.Count; ++i)
        {
            if(soe.subscribedByISubscriberToEvent[i]!=null)
                soe.subscribedByISubscriberToEvent[i].OnEventNotification(t1,t2);
        }
    }
